# -*- coding: utf-8 -*-
"""
Created on Wed Aug 10 16:01:47 2022
...
def Table_Prep():
    df = pd.read_csv(r'C:\Users\33o\Documents\Python Scripts\data\master_gas_consumption_table.csv')
...
def Model_Prep(df):
    ...
    df.drop(['year','FIPS','emp','ap'],axis=1,inplace=True)
    x = df.drop('county_consumption',axis=1)
    y = df['county_consumption']
...
refi_df, cols= Table_Prep()
correlated = Correlation(refi_df)
x_train,y_train,x_test,y_test = Model_Prep(refi_df)
...
fig.suptitle('Gas Consumption Model')

ax1.scatter(y_p,y_train)
ax1.set_ylabel('predicted gas consumption')
ax1.set_xlabel('true gas consumption')
ax1.set_xlim(-5000,280000000)
ax1.set_ylim(-5000,280000000)
ax1.title.set_text('Training Set')
b, a = np.polyfit([0,1], [0,1], deg=1)
xseq = np.linspace(0, 300000000, num=100)
ax1.plot(xseq, a + b * xseq, color="k", lw=2.5,linestyle='--')
ax1.text(150000000,50000000,'R^2 = '+str(round(r2_score(y_train, y_p),4)),bbox=dict\
...
ax2.set_ylabel('predicted gas consumption')
ax2.set_xlabel('true gas consumption')
ax2.set_xlim(-5000,280000000)
ax2.set_ylim(-5000,280000000)
ax2.title.set_text('Test Set')
d, c = np.polyfit([0,1], [0,1], deg=1)
xseq = np.linspace(0, 300000000, num=100)
ax2.plot(xseq, c + d * xseq, color="r", lw=2.5,linestyle='--')
ax2.text(150000000,50000000,'R^2 = '+str(round(r2_score(y_test, y_pred),4)),bbox=dict\
         (facecolor='r', alpha=0.5),size='large')
    
xlabels = ['{:,.0f}'.format(x) + 'M' for x in ax2.get_xticks()/1000000]
